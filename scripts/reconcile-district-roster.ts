 ...
 * Plus: district mismatches vs our county-based districts.ts mapping.
 *
...
async function main() {
  ...
  const districtsJsonPath = new URL("../conferences/rio-texas/districts.json", import.meta.url).pathname;
  const districtsJson = JSON.parse(readFileSync(districtsJsonPath, "utf8"));
  districtsJson.roster = byGcfa2;
  writeFileSync(districtsJsonPath, JSON.stringify(districtsJson, null, 2) + "\n");
  console.log(`District override map: conferences/rio-texas/districts.json (${Object.keys(byGcfa2).length} churches)`);
